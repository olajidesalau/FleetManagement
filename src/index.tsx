  ...
  return async (c: any, next: any) => {
    ...
    if (!user || !isAdminRole(user.role)) {
      if (c.req.header('Accept')?.includes('text/html')) {
        ...
        const resource = path.includes('/vehicles/') ? 'vehicle' : path.includes('/customers/') ? 'customer' : 'driver'
        const backPath = resource === 'vehicle' ? '/vehicles' : resource === 'customer' ? '/customers' : '/drivers'
        return c.html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Access denied - Snow Fleet Management</title><link rel="stylesheet" href="/static/style.css"></head><body><nav class="topbar"><div class="nav-shell"><div class="brand-lockup"><span class="brand-mark">S</span><span>Snow Fleet <em>Management</em></span></div><div class="primary-nav"><a href="/">Overview</a><a href="/drivers">Drivers</a><a href="/vehicles">Vehicles</a><a href="/customers">Customers</a><a href="/messages">Messages</a></div><div class="user-menu"><a href="/profile">Account</a></div></div></nav><main class="fleet-dashboard"><section class="panel access-denied-panel"><p class="eyebrow">Fleet permissions</p><h1>Access denied</h1><p class="access-denied-message">Insufficient permissions. Only Admin and Fleet Manager accounts can edit this ${resource}.</p><a class="button button-secondary" href="${backPath}">Back to ${resource}s</a></section></main></body></html>`, 403)
      }
      return c.json({ error: 'Forbidden - insufficient permissions' }, 403)
    }
    await next()
...
app.post('/api/fleet/drivers/:driverId', authenticate, requireAdmin(), async (c) => {
...
app.get('/admin/drivers', authenticate, requireAdmin(), async (c) => {
...
app.get('/vehicles/:vehicleId', async (c) => {
  const vehicleId = c.req.param('vehicleId')
  const vehicle = await c.env.DB.prepare(`SELECT v.id, v.vehicle_reference, v.registration_number, v.vehicle_type, v.make, v.model, v.year, v.mileage, v.temperature_controlled, v.target_temperature_min, v.target_temperature_max, v.current_temperature, v.status, v.last_seen_at, r.route_reference, r.origin, r.destination FROM vehicles v LEFT JOIN fleet_routes r ON r.vehicle_id = v.id AND r.status NOT IN ('delivered', 'cancelled') WHERE v.vehicle_reference = ? OR v.id = ? LIMIT 1`).bind(vehicleId, Number(vehicleId) || 0).first() as any
...
app.get('/drivers/:driverId/edit', authenticate, requireAdmin(), async (c) => {
  const driverId = c.req.param('driverId')
...
app.get('/customers/:customerId', async (c) => {
  ...
  const prospectusCustomers: Record<string, any> = {
    ...
    'PRO-0101': { full_name: 'Bidfood Liverpool', email: 'LiverpoolCustomerService@bidfood.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0102': { full_name: 'Bidfood Manchester', email: 'manchestercustomerservice@bidfood.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0103': { full_name: 'Best Food Logistics', email: 'contact@bestfoodlogistics.com', status: 'prospect', role: 'customer' },
    'PRO-0104': { full_name: 'NHS Supply Chain', email: 'enquiries@supplychain.nhs.uk', status: 'prospect', role: 'customer' },
    'PRO-0105': { full_name: 'Brakes Warrington', email: 'contact@brake.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0106': { full_name: 'Peter Green Chilled', email: 'sales@petergreenchilled.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0107': { full_name: 'Americold UK', email: 'contact@americold.com', status: 'prospect', role: 'customer' },
    'PRO-0108': { full_name: 'Lineage Logistics UK', email: 'contact@onelineage.com', status: 'prospect', role: 'customer' },
    'PRO-0109': { full_name: 'Culina Group', email: 'contact@culina.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0110': { full_name: 'Reed Boardall', email: 'contact@reedboardall.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0111': { full_name: 'NewCold', email: 'contact@newcold.com', status: 'prospect', role: 'customer' },
    'PRO-0112': { full_name: 'DFDS Logistics', email: 'contact@dfds.com', status: 'prospect', role: 'customer' },
    'PRO-0113': { full_name: 'Fowler Welch', email: 'contact@fowlerwelch.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0114': { full_name: 'Fresh Direct', email: 'contact@freshdirect.co.uk', status: 'prospect', role: 'customer' },
    'PRO-0115': { full_name: 'Booker Wholesale', email: 'contact@booker.co.uk', status: 'prospect', role: 'customer' },
  ...
  const customer = await c.env.DB.prepare(`SELECT id, full_name, email, phone, postcode, role, status, created_at FROM users WHERE (id = ? OR email = ?) AND role = 'customer' LIMIT 1`).bind(Number(customerId) || 0, customerId).first() as any || prospectusCustomers[customerId]
  if (!customer) return c.redirect('/customers')
  const routes = customer.id ? await c.env.DB.prepare(`SELECT route_reference, origin, destination, status, scheduled_departure, estimated_arrival FROM fleet_routes WHERE customer_id = ? ORDER BY scheduled_departure DESC LIMIT 50`).bind(customer.id).all() : { results: [] }
  const currentUser = c.get('user') as any
  const escapeHtml = (value: unknown) => String(value ?? '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[character] || character))
  const routeMarkup = (routes.results || []).map((route: any) => `<a class="conversation-list-item" href="/routes/${encodeURIComponent(route.route_reference)}"><span class="conversation-avatar">R</span><span class="conversation-summary"><strong>${escapeHtml(route.route_reference)}</strong><span>${escapeHtml(route.origin)} to ${escapeHtml(route.destination)}</span></span><span class="conversation-meta"><small>${escapeHtml(route.status)}</small></span></a>`).join('') || '<div class="messages-empty"><strong>No routes linked</strong><span>Routes allocated to this customer will appear here.</span></div>'
  const editLink = isAdminRole(currentUser?.role) ? `<a class="button button-primary" href="/customers/${encodeURIComponent(customerId)}/edit">Edit customer</a>` : ''
  return c.html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escapeHtml(customer.full_name)} - Snow Fleet Management</title><link href="/static/style.css" rel="stylesheet"></head><body><nav class="topbar"><div class="nav-shell"><div class="brand-lockup"><span class="brand-mark">S</span><span>Snow Fleet <em>Management</em></span></div><div class="primary-nav"><a class="active" href="/">Overview</a><a href="/routes">Routes</a><a href="/vehicles">Vehicles</a><a href="/temperature">Temperature</a><a href="/drivers">Drivers</a><a href="/customers">Customers</a><a href="/alerts">Alerts</a><a href="/monitoring">Monitoring</a><a href="/traffic">Traffic</a><a href="/messages">Messages</a></div><div class="user-menu"><a href="/profile">${escapeHtml(currentUser?.email || 'Account')}</a></div></div></nav><main class="fleet-dashboard customer-detail-page"><div class="page-heading"><div><p class="eyebrow">Fleet management / customer detail</p><h1>${escapeHtml(customer.full_name)}</h1><p class="header-copy">Review customer contact information, delivery requirements, and route history.</p></div><div class="header-actions"><a class="button button-secondary" href="/customers">All customers</a>${editLink}</div></div><section class="metric-grid"><article class="metric-card metric-card-accent"><span class="metric-label">Customer status</span><strong>${escapeHtml(customer.status || 'active')}</strong><span class="metric-note">Account status</span></article><article class="metric-card"><span class="metric-label">Total routes</span><strong>${routes.results?.length || customer.route_count || 0}</strong><span class="metric-note">Linked delivery routes</span></article><article class="metric-card"><span class="metric-label">Active routes</span><strong>${(routes.results || []).filter((route: any) => !['delivered', 'cancelled'].includes(route.status)).length}</strong><span class="metric-note">Not yet delivered</span></article><article class="metric-card"><span class="metric-label">Postcode</span><strong>${escapeHtml(customer.postcode || 'N/A')}</strong><span class="metric-note">Delivery area</span></article></section><div class="detail-grid"><section class="panel detail-panel"><div class="panel-heading"><div><p class="eyebrow">Customer record</p><h2>Contact details</h2></div><span class="pill pill-green">${escapeHtml(customer.role || 'customer')}</span></div><div class="detail-facts"><div><span>Organisation</span><strong>${escapeHtml(customer.full_name)}</strong></div><div><span>Email</span><strong>${escapeHtml(customer.email)}</strong></div><div><span>Phone</span><strong>${escapeHtml(customer.phone || 'Not recorded')}</strong></div><div><span>Postcode</span><strong>${escapeHtml(customer.postcode || 'Not recorded')}</strong></div><div><span>Registered</span><strong>${escapeHtml(customer.created_at || 'Prospect record')}</strong></div></div></section><section class="panel detail-panel"><div class="panel-heading"><div><p class="eyebrow">Delivery requirements</p><h2>Route history</h2></div></div><div class="conversation-list">${routeMarkup}</div></section></div></main><script src="/static/app.js"></script></body></html>`)
  */
})
...
app.get('/admin/providers', authenticate, requireAdmin(), async (c) => {
  const status = c.req.query('status') || ''
...
app.get('/admin/bookings', authenticate, requireAdmin(), async (c) => {
...
app.get('/messages', authenticate, async (c) => {
  ...
  const conversationMarkup = conversations.length
    ? conversations.map(conversation => `<a class="conversation-list-item" href="/messages/conversation/${encodeURIComponent(String(conversation.id))}"><span class="conversation-avatar">${escapeHtml(String(conversation.other_user_name || 'U').slice(0, 1).toUpperCase())}</span><span class="conversation-summary"><strong>${escapeHtml(conversation.other_user_name || 'Fleet contact')}</strong><span>${escapeHtml(conversation.last_message || 'No messages yet')}</span></span><span class="conversation-meta"><small>${escapeHtml(conversation.last_message_date || '')}</small></span></a>`).join('')
    : '<div class="messages-empty"><strong>Your inbox is clear.</strong><span>Choose a contact to start a conversation.</span></div>'
  const contactMarkup = contacts.length
    ? contacts.map(contact => `<a href="/messages/new?to=${encodeURIComponent(String(contact.id))}"><span class="contact-avatar">${escapeHtml(String(contact.full_name || 'U').slice(0, 1).toUpperCase())}</span><span><b>${escapeHtml(contact.full_name || contact.email)}</b><small>${escapeHtml(contact.driver_reference || contact.email)}</small></span><span>Message</span></a>`).join('')
    : '<div class="messages-empty"><span>No active contacts are available.</span></div>'
  return c.html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Messages - Snow Fleet Management</title><link href="/static/style.css" rel="stylesheet"><link href="/static/login-theme.css" rel="stylesheet"></head><body><nav class="topbar"><div class="nav-shell"><div class="brand-lockup"><span class="brand-mark">S</span><span>Snow Fleet <em>Management</em></span></div><div class="primary-nav"><a href="/">Overview</a><a href="/routes">Routes</a><a href="/vehicles">Vehicles</a><a href="/temperature">Temperature</a><a href="/drivers">Drivers</a><a href="/customers">Customers</a><a href="/alerts">Alerts</a><a href="/monitoring">Monitoring</a><a href="/traffic">Traffic</a><a class="active" href="/messages">Messages</a></div><div class="user-menu"><a href="/profile">${escapeHtml(user.email || 'Account')}</a><a class="avatar" href="/profile" aria-label="Open my profile">FM</a></div></div></nav><main class="fleet-dashboard messages-page"><div class="page-heading"><div><p class="eyebrow">Communication hub</p><h1>Messages</h1><p class="header-copy">Coordinate routes, deliveries, vehicles, and customer requirements.</p></div><div class="header-actions"><span class="sync-status"><span class="status-dot status-dot-live"></span>${conversations.length} conversations</span></div></div><div class="messages-layout"><section class="panel conversation-list-panel"><div class="panel-heading"><div><p class="eyebrow">Inbox</p><h2>Recent conversations</h2></div><span class="count-badge">${conversations.length}</span></div><div class="conversation-list">${conversationMarkup}</div></section><section class="panel message-intro-panel"><p class="eyebrow">Available contacts</p><h2>Start a conversation</h2><div class="message-contact-groups">${contactMarkup}</div></section></div></main><script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script><script src="/static/app.js"></script></body></html>`)
})
...
app.get('/messages/new', authenticate, async (c) => {
  try {
    ...
    const escapeHtml = (value: unknown) => String(value ?? '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[character] || character))
    const messageMarkup = (messages.results || []).map((message: any) => `<div class="message-bubble"><strong>${escapeHtml(message.sender_name)}</strong><p>${escapeHtml(message.message_text)}</p><small>${escapeHtml(message.created_at)}</small></div>`).join('') || '<p class="messages-empty">No messages yet. Say hello!</p>'
    return c.html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Conversation - Snow Fleet Management</title><link href="/static/style.css" rel="stylesheet"></head><body><nav class="topbar"><div class="nav-shell"><div class="brand-lockup"><span class="brand-mark">S</span><span>Snow Fleet <em>Management</em></span></div><div class="primary-nav"><a href="/">Overview</a><a href="/routes">Routes</a><a href="/vehicles">Vehicles</a><a href="/temperature">Temperature</a><a href="/drivers">Drivers</a><a href="/customers">Customers</a><a href="/alerts">Alerts</a><a href="/monitoring">Monitoring</a><a href="/traffic">Traffic</a><a class="active" href="/messages">Messages</a></div><div class="user-menu"><a href="/profile">${escapeHtml(user.email || 'Account')}</a></div></div></nav><main class="fleet-dashboard conversation-page"><div><div class="page-heading"><div><p class="eyebrow">Profile messaging</p><h1>Conversation with ${escapeHtml(otherUser.full_name || otherUser.email)}</h1><p class="header-copy">Send a message securely to this profile.</p></div><a class="button button-secondary" href="/messages">Back to messages</a></div><section class="panel conversation-card"><div class="conversation-messages">${messageMarkup}</div><form data-conversation-form data-receiver-id="${receiverId}"><input name="message_text" maxlength="4000" placeholder="Type your message..." required><button type="submit">Send</button></form></section></div></main><script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script><script src="/static/app.js"></script></body></html>`)
  } catch {
...
app.get('/messages/conversation/:conversationId', authenticate, async (c) => {
  try {
    ...
    const escapeHtml = (value: unknown) => String(value ?? '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[character] || character))
    const messageMarkup = (msgs.results || []).map((message: any) => `<div class="message-bubble"><strong>${escapeHtml(message.sender_name)}</strong><p>${escapeHtml(message.message_text)}</p><small>${escapeHtml(message.created_at)}</small></div>`).join('') || '<p class="messages-empty">No messages yet. Say hello!</p>'
    return c.html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Conversation - Snow Fleet Management</title><link href="/static/style.css" rel="stylesheet"></head><body><nav class="topbar"><div class="nav-shell"><div class="brand-lockup"><span class="brand-mark">S</span><span>Snow Fleet <em>Management</em></span></div><div class="primary-nav"><a href="/">Overview</a><a href="/routes">Routes</a><a href="/vehicles">Vehicles</a><a href="/temperature">Temperature</a><a href="/drivers">Drivers</a><a href="/customers">Customers</a><a href="/alerts">Alerts</a><a href="/monitoring">Monitoring</a><a href="/traffic">Traffic</a><a class="active" href="/messages">Messages</a></div><div class="user-menu"><a href="/profile">${escapeHtml(user.email || 'Account')}</a></div></div></nav><main class="fleet-dashboard conversation-page"><div><div class="page-heading"><div><p class="eyebrow">Profile messaging</p><h1>Conversation with ${escapeHtml(otherUser?.full_name || otherUser?.email || 'Fleet contact')}</h1><p class="header-copy">Send a message securely to this profile.</p></div><a class="button button-secondary" href="/messages">Back to messages</a></div><section class="panel conversation-card"><div class="conversation-messages">${messageMarkup}</div><form data-conversation-form data-receiver-id="${escapeHtml(otherId)}"><input name="message_text" maxlength="4000" placeholder="Type your message..." required><button type="submit">Send</button></form></section></div></main><script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script><script src="/static/app.js"></script></body></html>`)
  } catch (error: any) {
