...
app.post('/api/fleet/routes', async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare(`INSERT INTO fleet_routes (route_reference, customer_id, driver_id, vehicle_id, origin, destination, scheduled_departure, estimated_arrival, status, cargo_description, traffic_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?)`).bind(form.route_reference, form.customer_id ? Number(form.customer_id) : null, form.driver_id ? Number(form.driver_id) : null, form.vehicle_id ? Number(form.vehicle_id) : null, form.origin, form.destination, form.scheduled_departure, form.estimated_arrival || null, form.cargo_description || null, form.traffic_status || 'clear').run()
...
app.get('/deliveries/:routeId', async (c) => {
  ...
  let proofImage = ''
  try {
    const activity = await c.env.DB.prepare(`SELECT proof_photo_data FROM route_activity_history WHERE route_id = ? AND activity_type = 'delivered' ORDER BY recorded_at DESC LIMIT 1`).bind(safeRouteId).first() as any
    proofImage = activity?.proof_photo_data || ''
  } catch {}
  const completedMarkup = proofImage ? `<section class="panel delivery-confirmed"><span class="delivery-check">✓</span><div><p class="eyebrow">Completed delivery</p><h2>Electronic proof recorded</h2><p>The delivery proof photo is stored with the route activity.</p><img class="proof-photo" src="${proofImage}" alt="Electronic proof of delivery"></div></section>` : ''
  return c.html(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Delivery confirmation</title><link rel="stylesheet" href="/static/style.css"></head><body><nav class="topbar"><div class="nav-shell"><div class="brand-lockup"><span class="brand-mark">S</span><span>Snow Fleet <em>Management</em></span></div><div class="primary-nav"><a href="/">Overview</a><a href="/routes">Routes</a><a href="/vehicles">Vehicles</a><a href="/drivers">Drivers</a><a href="/customers">Customers</a><a href="/monitoring">Monitoring</a><a href="/traffic">Traffic</a></div><div class="user-menu"><a href="/auth/login">Login</a><a href="/auth/register">Register</a></div></div></nav><main class="fleet-dashboard"><div class="page-heading"><div><p class="eyebrow">Delivery confirmation</p><h1>Route ${safeRouteId}</h1><p class="header-copy">Record the time goods were delivered and include an electronic proof photo.</p></div><a class="button button-secondary" href="/routes">← Routes</a></div>${completedMarkup}<section class="panel delivery-form-panel"><div class="panel-heading"><div><p class="eyebrow">Driver action</p><h2>Confirm delivery</h2></div></div><form method="post" action="/api/fleet/routes/${safeRouteId}/deliver" enctype="multipart/form-data" class="delivery-form"><label>Delivery timestamp<input name="actual_arrival" type="datetime-local" required></label><label>Delivery notes<textarea name="notes" placeholder="Condition of goods, recipient name, or delivery exception"></textarea></label><label>Proof of delivery reference<input name="proof_reference" placeholder="Optional POD reference"></label><label>Electronic proof photo<input name="proof_photo" type="file" accept="image/jpeg,image/png,image/webp" required><small>Upload a clear delivery photo, maximum 2 MB.</small></label><button class="button button-primary" type="submit">Mark goods delivered</button></form></section></main></body></html>`)
})
